...
def face(color: list, line_color: list, line_width: int, *poses: list):
    ...
    faces_lines = n.vstack((faces_lines, n.array([[float(line_color[0]), float(line_color[1]), float(line_color[2]), float(l_alpha), float(line_width)]], dtype=n.float32)))
...
def pyramid(line_color: list, line_width: float, color: list, pos_primary, *poses: list):
    face(color, line_color, line_width, *poses)
    for p in range(len(poses)):
        next = int((p+1)%len(poses))
        face(color, line_color, line_width, poses[next], poses[p], pos_primary)
...
def update():
    ...
    if result is not None:
        ...
        for i in sorted_indices:
            ...
            pygame.draw.polygon(screen, (int(faces_lines[original_face_idx][0]), int(faces_lines[original_face_idx][1]), int(faces_lines[original_face_idx][2])), final[i], int(faces_lines[original_face_idx][4]))
        return True
